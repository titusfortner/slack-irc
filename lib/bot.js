...
class Bot {
  ...
  parseText(text) {
    return text
      ...
      .replace(/<#(C\w+)\|?(\w+)?>/g, (match, channelId, readable) => {
        const { name } = this.slack.rtm.dataStore.getChannelById(channelId);
        return readable || `#${name}`;
      })
      .replace(/<@(U\w+)\|?(\w+)?>/g, (match, userId, readable) => {
        const { name } = this.slack.rtm.dataStore.getUserById(userId);
        return readable || `@${name}`;
      })
  ...
  sendToIRC(message) {
    ...
    const channelName = channel.isChannel ? `#${channel.name}` : channel.name;
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      const iconUrl = author !== this.nickname ?
      ...
      this.slack.web.chat.postMessage(slackChannel.id, mappedText, {
        username: author,
        parse: 'full',
        icon_url: iconUrl
      }, () => null);
